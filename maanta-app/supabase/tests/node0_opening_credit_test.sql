...
-- Scenario E: the cap is PER NODE (migration 20260730120000).
-- One node filling its allowance must not exhaust the next node's. Before the
-- fix the count was global, so moving node0_launch_node to a new mall after the
-- first node filled up left the new node's promo dead on arrival — activations
-- there silently granted nothing.
DO $$
DECLARE
  v_a UUID; v_b UUID;
