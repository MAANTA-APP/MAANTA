...
COMMENT ON TABLE public.nodes IS
  'Registry of MAANTA nodes (malls). Source of truth for which nodes exist. `id` is an opaque grandfathered key — render `label`, never `id`. See D72.';
COMMENT ON COLUMN public.nodes.id IS
  'Opaque stable key, grandfathered from the pre-D72 deals.node / merchants.node text values. Never display it; never change it.';
...
-- follow rather than orphaning — the exact failure D72 was opened for.
...
BEGIN
  IF NEW.id IS DISTINCT FROM OLD.id THEN
    RAISE EXCEPTION
      'node_id_immutable: nodes.id is an opaque key referenced by deals.node and merchants.node. Change label/short_label instead (D72).';
  END IF;
  RETURN NEW;
END;
$$;
