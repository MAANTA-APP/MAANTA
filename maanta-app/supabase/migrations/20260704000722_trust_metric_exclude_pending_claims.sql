-- Fix session 2026-07-04 (founder-confirmed): the 30-day trust ratio must count
-- terminal-state redemptions only (success / failed / flagged). Pending claims
-- are intent, not outcomes — counting them meant a merchant with 3 claims and
-- 1 verification computed trust 0.467 and was auto-hidden from the feed
-- (diagnosed empirically in the 2026-07-04 D-003 Build session; root cause:
-- redemptions.redeemed_at DEFAULT now() stamps pending rows at claim time,
-- pulling them into the window).
-- Only the counting WHERE clause changes. Weights, thresholds, visibility
-- rules, retraining-task behaviour: unchanged. Zero-history merchants still
-- get benefit of the doubt (terminal total = 0 -> v_r = 1.0, v_f = 0.0).
...
BEGIN
  ...
  WHERE merchant_id=p_merchant_id
    AND status IN ('success','failed','flagged')   -- terminal states only; pending excluded
