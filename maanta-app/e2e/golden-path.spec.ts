 ...
 *   - E2E_DEAL_PATH             path to a claimable seeded deal (default derived
 *                               from /feed)
...
test.describe("golden path (browser)", () => {
  ...
  test("shopper claims a deal and sees a 6-digit code ticket", async ({ browser }) => {
    const ctx = await browser.newContext({ storageState: SHOPPER_STORAGE });
  ...
  test("merchant verifies the code: fee disclosed, then COLLECT FROM SHOPPER + Verified", async ({
    ...
    const shopperCtx = await browser.newContext({ storageState: SHOPPER_STORAGE });
    ...
    const merchantCtx = await browser.newContext({ storageState: MERCHANT_STORAGE });
    ...
    await expect(page.getByText(/Collect from shopper/i)).toBeVisible();
    await expect(page.getByText(/success fee/i)).toBeVisible();
    // The collect amount and the KES 30 fee are both present and distinct.
    await expect(page.getByText(/KES\s*30/)).toBeVisible();
    await merchantCtx.close();
  });

  test("negative: an invalid code is rejected with no fee", async ({ browser }) => {
    const ctx = await browser.newContext({ storageState: MERCHANT_STORAGE });
    const page = await ctx.newPage();
    await page.goto("/merchant/redeem");
...
async function claimFirstDeal(page: Page): Promise<string> {
  await page.goto(process.env.E2E_DEAL_PATH ?? "/feed");
