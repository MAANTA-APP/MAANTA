...
function render(claimedAt: string | null, arrivedAt: string | null) {
  return renderToStaticMarkup(
    createElement(FastVisitPanel, { claimedAt, arrivedAt })
  );
}
...
describe("FastVisitPanel states", () => {
  ...
  it("renders NOTHING for a late arrival — normal claim experience, no shaming", () => {
    ...
    expect(render(claimed, arrived)).toBe("");
  });
  ...
  it("never uses amber — the reward is not an action and not a credential", () => {
    ...
    for (const html of [
      ...
      render(new Date(Date.now() - 20 * MIN).toISOString(), new Date(Date.now() - 15 * MIN).toISOString()),
    ]) {
