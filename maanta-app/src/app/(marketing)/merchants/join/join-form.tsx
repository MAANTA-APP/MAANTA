 ...
 *
 * Split out of `page.tsx` so the route can export `metadata`: a `"use client"`
 * module cannot, and while this whole page was a client component the route
 * inherited the root-default title and description — which is how it ended up in
 * the sitemap at priority 0.8 advertising "Maanta — The mall, made live."
 * (GAP-06). The client boundary is real — `useState`, `useRouter` and
 * `trackMarketing` all need it — so it moves down one level rather than away.
 * `waitlist/page.tsx` + `waitlist/waitlist-form.tsx` is the same shape.
 *
 * Field labels are verified live and deliberately unchanged (`copy/merchants.md`
 * §4).
 *
 * **The phone number survives the handoff.** This form previously collected a
