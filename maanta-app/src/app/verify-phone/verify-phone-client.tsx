...
/**
 * The interactive half of `/verify-phone`.
 *
 * **The strategy gate deliberately does not live here** — see `page.tsx`, which
 * is a server component for exactly that reason. `phoneOtpEnabled()` requires
 * *both* `MAANTA_AUTH_STRATEGY` and `NEXT_PUBLIC_MAANTA_AUTH_STRATEGY` to be
 * `clerk`, and the first of those is not inlined into a client bundle, so it
 * reads as `undefined` in the browser and the gate could never open here. That
 * was drift **D80**. Anything in this file that needs the resolved strategy
 * takes it as a prop.
 */
...
function VerifyPhoneUnavailable({ loginHint }: { loginHint: string }) {
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-mobile flex-col px-5 pb-10 pt-6">
      ...
      <p className="mt-2 text-sm text-muted">{loginHint}</p>
...
export function ClerkVerifyPhonePage() {
  return (
    <>
      <SignedIn>
...
export function SupabaseVerifyPhonePage({ loginHint }: { loginHint: string }) {
  return (
    <>
      <SupabaseSignedIn>
        <Suspense fallback={null}>
          <VerifyPhoneUnavailable loginHint={loginHint} />
        </Suspense>
      </SupabaseSignedIn>
      <SupabaseSignedOut>
        <SupabaseRedirectToSignIn />
