/**
 * Canonical country calling-code dataset for the phone country picker
 * (`PhoneField` in `src/components/ui/inputs.tsx`).
 *
 * Ordering rule: Kenya, Norway, United Kingdom pinned first (in that order),
 * then every other country alphabetically. Pinned countries appear once —
 * they are excluded from the alphabetical remainder. Guarded by
 * `src/lib/__tests__/country-codes.test.ts`.
 *
 * Dial codes are E.164 country calling codes; several countries share one
 * (e.g. +1, +7, +44 territories collapse to distinct ISO rows), so rows are
 * keyed by ISO 3166-1 alpha-2, never by dial code.
 */
export type CountryOption = {
  /** ISO 3166-1 alpha-2 — the unique key for a row. */
  iso2: string;
  name: string;
  /** E.164 calling code with leading "+". Not unique across countries. */
  dialCode: string;
};

export const PINNED_ISO2 = ["KE", "NO", "GB"] as const;
...
const ALL_COUNTRIES: CountryOption[] = [
  ...
  { iso2: "CG", name: "Congo (Republic)", dialCode: "+242" },
...
const pinned = PINNED_ISO2.map((iso2) => {
  const match = ALL_COUNTRIES.find((c) => c.iso2 === iso2);
  if (!match) throw new Error(`Pinned country ${iso2} missing from dataset`);
  return match;
});

const rest = ALL_COUNTRIES.filter(
  (c) => !(PINNED_ISO2 as readonly string[]).includes(c.iso2)
).sort((a, b) => a.name.localeCompare(b.name, "en"));

/** Picker order: KE, NO, GB pinned, then everything else A–Z. */
