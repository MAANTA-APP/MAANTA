...
/**
 * Guard for drift **D59** — user input interpolated into a PostgREST filter
 * expression.
 *
...
describe("orIlikeAny keeps user input inside the value (D59)", () => {
  ...
  it("fails on an interpolated .or( anywhere under src/", () => {
    ...
    expect(
      ...
      "`.or()` takes PostgREST's filter DSL, not a bound value — an interpolated\n" +
        "template literal there is injectable (D59). Build the expression with\n" +
