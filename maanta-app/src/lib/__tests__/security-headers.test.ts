...
/**
 * Guard for drift **D74** — the app shipped no response security headers.
 *
...
beforeAll(async () => {
  ...
  expect(config.headers, "next.config.mjs must declare a headers() block (D74)").toBeTypeOf(
    "function"
  );
...
describe("every response carries the baseline hardening headers (D74)", () => {
...
describe("the CSP is honest about what it is", () => {
  it("ships Report-Only, and does not claim to be enforcing", () => {
    ...
    expect(
      ...
      "Promoting the CSP to enforcing is a separate, browser-verified change (D74).\n" +
