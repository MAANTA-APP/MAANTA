...
statement closes it. In the other direction, `20260818120000_deal_categories.sql`
and `20260818130000_demo_reseed_categories.sql` are in the repo and **not
applied**, so shopper category filtering cannot work on production until a human
runs them — **D116**, open. The `claim_deal` pause gate is **live** (**D25**
...
| `maanta-app/supabase/migrations/` | Version-controlled migration history — authoritative for DB behavior (ledger reconciled with prod at 87/87 on 2026-08-08 — drift **D24** closed 2026-08-05; the **D73** per-node cap reland `20260807160000` and **D74** cofounder read policies `20260807161000` applied to production 2026-08-08 and read back. **Out in two directions since 2026-08-18: `20260816020000` is applied but unrecorded (D107), and `20260818120000` + `20260818130000` are in the repo but unapplied (D116)**) |
...
| Am I touching deal categories? | `docs/skills/deal-categories.md` — the taxonomy is founder-locked, uncategorised is a real state, and the column is not on production yet (**D116**) |
