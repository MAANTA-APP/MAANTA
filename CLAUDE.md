...
still on. **The migration ledger reconciles at 110/110 as of 2026-09-03** (direct read of production `supabase_migrations.schema_migrations` — 110 rows, latest `20260903140000`, against 110 files in `supabase/migrations/`, full version+name diff identical; the 108th–110th are the three 2026-09-03 migrations — `20260903120000` claim allocation cap (**D223**/D236), `20260903130000` blacklist enforcement (**D171**/**D238**), `20260903140000` merchant tenant-policy repair (**D168**) — present under their repo versions, so no minted-version repair was needed this round. The founder caught an engineering recap that was about to have them reapplied: **read the ledger before acting, every time**). Previously **107/107 as of 2026-09-01** (re-verified by direct read of production `supabase_migrations.schema_migrations` — 107 rows, latest `20260830120000`, against 107 files in `supabase/migrations/`; the 106th and 107th are the 2026-08-30 fee-reporting and grant-ratchet migrations, **D169** closed 2026-09-01 with its MCP-minted `20260831081622` repaired to the repo-owned `20260830120000` — twelve for twelve). Previously **105/105 as of 2026-08-27** (D206's `20260827120000` — the active-deal cap now holds on entry into slot occupancy, not only on INSERT; before it the two 2026-08-26 dark-deploy migrations `20260826120000` (Fast Visit + Points, feature-flagged OFF) and `20260826130000` (merchant QR queue) took it to 104/104. Prior: 102/102 on 2026-08-25 with D164's `20260824130000`; it stood at 101/101 on 2026-08-24 after D162's `20260824120000`, and at 100/100 on 2026-08-23 after D154's `20260823120000` and D158's `20260823130000`, and at 98/98 on 2026-08-20) (**D24**
...
`schema_migrations` and this repo's `supabase/migrations/` agree on all 105
...
| `maanta-app/supabase/migrations/` | Version-controlled migration history — authoritative for DB behavior. **Ledger reconciles with prod at 110/110 as of 2026-09-03**, verified by full version+name read-back diff (the three 2026-09-03 migrations present under their repo versions; **D24** closed 2026-08-05, **D107** closed 2026-08-18, **D121** closed 2026-08-19 when the security branch merged and the repo finally contained every applied migration; **D147** added `20260820120000` on 2026-08-20, **D154** `20260823120000` and **D158** `20260823130000` on 2026-08-23, **D162** `20260824120000` on 2026-08-24, **D164** `20260824130000` on 2026-08-25). Read the **ledger**, not this directory, before picking a version — for two days it under-reported the high-water mark by five |
...
| Which document do I read, and is it current? | `docs/maanta-documentation-register.md` — every audience-facing and operational document with its owner, status, source of truth, last-verified date, review trigger and confidentiality. Also carries the P0–P3 gap matrix. Read it before writing a new doc; the gap may already be recorded |
| How is Merchant 01 actually run? | `docs/ops/merchant-01-pilot-runsheet.md` (the end-to-end sequence), then `docs/ops/node0-known-limitations.md` (what is live, dark or unproven) and `docs/ops/evidence-classification-guide.md` (how the result gets counted) |
| Am I touching the admin console or the founder command centre? | `docs/skills/admin-founder-command-centre-2026-09-03.md` — the task-oriented admin IA, the Action Queue rules (`lib/admin-action-queue.ts`), Merchant 360, the visit funnel (`lib/visit-funnel.ts`), the founder verdict readings (`lib/founder-command-centre.ts`), and the claim-allocation vocabulary (**D233**: `max_claims` is the number of shopper claims that may be issued, never a redemption limit — render it through `lib/claim-allocation.ts`) |
| Am I touching a public claim (premises, payment, staffing, support, the feed) or a public form? | `docs/skills/public-claims-and-form-safety-2026-09-04.md` — every such sentence has one source (`lib/marketing/live-claims.ts`, `facts.ts` `PAYMENT_AVAILABILITY`, `forms.ts` `FORM_STATUS`) and a banned-string sweep (`public-claims.test.ts`). `/contact` and `/waitlist` are non-collecting until FC1 is ruled (**D271**); MAANTA has no desk or address at BBS Mall (**D261**) |
