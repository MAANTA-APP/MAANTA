...
| D25 | open | code-outlier | 2026-07-30 | Core loop | The D24 numbering collision has already cost a fix: `maanta-app/supabase/migrations/20260730160000_restore_claim_deal_pause_gate.sql` is merged on `main`, but prod's ledger already holds `20260730160000`, so `supabase db push` will treat it as applied and **silently skip it forever**. Confirmed live: `claim_deal` on production contains no `deal_paused` branch, so a paused deal still accepts new claims while the merchant UI says "No new claims while paused" | Renumber the migration to an unused version ahead of the ledger, then push and re-check that `pg_get_functiondef` for `claim_deal` contains `deal_paused`. Until then the pause control is cosmetic on production. Do not close on the merge — close on the read-back | founder |
