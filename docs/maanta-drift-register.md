...
| D32 | closed | code-outlier | 2026-07-30 | Core loop | `deals_public_browse` filtered active + unexpired merchants but **not** `is_paused`, so a paused deal could still appear via the SQL discovery view even though `getLiveDeals` hid it and `claim_deal` would raise `deal_paused` — UI safety without SQL parity | `maanta-app/supabase/migrations/20260730190000_paused_deals_discovery_filter.sql` adds `is_paused IS NOT TRUE`; covered by scenarios A–C in `maanta-app/supabase/tests/claim_deal_pause_gate_test.sql`; semantics in `docs/skills/paused-deal-semantics.md`. D25 (RPC not live on prod) stays open until human `db push` | eng |
