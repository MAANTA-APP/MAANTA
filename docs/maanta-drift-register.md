...
| D25 | pending-deploy | code-outlier | 2026-07-30 | Core loop | The D24 numbering collision cost a fix: the paused-deal gate was merged on `main` as version `20260730160000`, which prod's ledger already held, so `supabase db push` treated it as applied and skipped it silently. Confirmed live on 2026-07-30: `claim_deal` on production contains no `deal_paused` branch, so the database still accepts claims on a paused deal. #146 has since hidden paused deals from the shopper rails, so the *feed* no longer advertises them — but the RPC is the enforcement point, and a deep link, a cached client or a direct API call still gets a claim | Renumbered past the ledger to `maanta-app/supabase/migrations/20260730170000_restore_claim_deal_pause_gate.sql`, so `db push` will now apply it; covered by `maanta-app/supabase/tests/claim_deal_pause_gate_test.sql`. **Not live until a human runs `supabase db push`** — the renumbering only makes it applicable, it does not apply it. Close only on the read-back: `pg_get_functiondef` for `claim_deal` must contain `deal_paused`. The app-layer filter in `maanta-app/src/lib/data.ts` narrows the exposure; it does not close this row | founder |
