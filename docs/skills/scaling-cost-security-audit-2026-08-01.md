...
Drift found is recorded first, as rows **D58–D66** in
...
- **The `.or()` injection (D59)** — reproduced by building the exact query with
...
2. **Adding a mall is a code change.** `NODES` is a hardcoded TypeScript array
   and the join key is the mall's display name (**D62**). This is the single
...
### 2.1 Nodes are code, and the key is a display label (D62)
...
remaining step is moving node selection onto the table, tracked on D62.
...
### 2.3 Queries that silently truncate as data grows (D63)
...
### 3.1 `service_role` is the default read path, and the code says otherwise — D58, Medium
...
### 3.2 PostgREST filter injection in admin customer search — D59, Medium
...
### 3.3 IntaSend top-up credit is forgeable with a single secret — D60, High (rail not yet live)
...
the `api_ref` this app issued, closes D60. If the contract differs, the failure
is safe by construction — top-ups 500 and retry rather than over-credit — but
M-Pesa will not work until it is reconciled.

### 3.4 Redemption codes come from a non-cryptographic PRNG — D66, Low
...
### 3.5 No response security headers at all — D64, Medium
...
- **The webhook failure paths (D61).** Both payment webhooks call
  ...
  writes.
- **`/api/sentry-example-api` (D65).** An unauthenticated `GET` whose entire
...
| # | Action | Why now | Row |
|---|---|---|---|
| 1 | ~~Verify the IntaSend invoice out of band before crediting a wallet~~ — **done 2026-08-02**, mutation-proven. Remaining: one sandbox STK push to confirm the endpoint contract, before E6 go-live | M-Pesa is not live yet, so this was free to fix and expensive later | D60 |
| 2 | ~~Promote nodes to a table; migrate `deals.node` / `merchants.node` to an FK~~ — **registry shipped 2026-08-02**, mutation-proven. Remaining: a human `supabase db push`, then move node *selection* onto the table | Contained at one node, a data-repair project at ten, and mall names will be on signage | D62 |
| 3 | ~~Fix the `.or()` construction and guard the pattern~~ — **done 2026-08-02**, mutation-proven; the guard now fails the build on any interpolated `.or(` | Small, self-contained, and one copy-paste from a shopper surface | D59 |
| 4 | ~~Add the `headers()` block~~ — **done 2026-08-02**, five headers enforced and verified on live responses. Remaining: watch the Report-Only CSP in a browser across auth, top-up and the map, then promote it | Four were zero-risk; CSP gets harder with every third-party origin added | D64 |
| 5 | Rate-limit the pre-auth webhook branches; add retention on `payment_webhook_failures`; gate the Sentry example route | Protects the alert channel the money path relies on | D61, D65 |
| 6 | Add the directory-walking route-guard test | Converts a convention into a mechanism before contributor count grows | §5.2 |
| 7 | Bound the `fraud_events` select and audit for other unbounded selects | Silent wrongness, and it arrives platform-wide rather than per node | D63 |
| 8 | Decide the `service_role` posture and write down whichever answer wins | Cheap now, structural later; blocks nothing but shapes everything after it | D58 |
...
| 10 | Swap `RANDOM()` for `gen_random_bytes` next time a migration touches `claim_deal` | Low risk today under good containment; free to fix opportunistically | D66 |
...
1. **`service_role` posture (D58)** — correct the docblock, or move shopper
   reads onto the RLS-governed client. Founder/eng call. Everything else in this
   document is a fix; this one is a direction.
2. **Node table timing (D62)** — before mall #2, or accept a harder migration
...
- **2026-08-01** — audit written. No code changed; findings recorded as
  **D58–D66**.
- **2026-08-02** — **D60 fixed** on founder instruction: the IntaSend webhook
  ...
  typecheck clean, **545 tests / 70 files** (from 525 / 69). D60 stays open
  pending a sandbox contract check. No other finding touched.

- **2026-08-02** — **D62 registry shipped** on founder instruction (§2.1, "What
  ...
  PostgreSQL harness — **84 migrations applied, 23/23 SQL suites passing**. D62
  stays open pending a human `supabase db push` and moving node selection onto
  the table.

- **2026-08-02** — **D59 closed and D64's baseline shipped** on founder
  ...
  marketing page, the merchant money surface, an API route and a 404. D64 stays
