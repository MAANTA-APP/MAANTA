...
| **D3** | **Multi-currency top-ups (KES/USD/EUR/GBP + live FX)** | `migrations/20260708231241_add_multicurrency_and_webhook_failure_log.sql`, `src/lib/currency.ts` (`SUPPORTED_CURRENCIES`), `src/app/api/topup/stripe/route.ts`. **API-level only — `topup-flow.tsx` exposes no currency selector (KES-only in practice)** | Frozen Scope do-not-build list still forbids "Multi-currency" | decide | Logged in repo (2026-07-08). Reconcile: either (a) narrow the do-not-build item to "no *user-facing* multi-currency; API accepts non-KES with FX→KES for future rails" and update Frozen Scope, or (b) remove the unused non-KES path. Recommend (a) |
