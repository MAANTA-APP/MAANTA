...
| 2026-07-16 | **`merchant_staff.can_topup` / `can_purchase` are NOT active billing authority.** Verification session confirmed the frozen "staff cannot touch billing/top-ups/boosts" rule holds, but *incidentally*: top-up/verify routes resolve the merchant by owner `user_id` (staff → 404), and boost RPCs are owner/admin-only. `can_topup` is a **dead field** (never read for enforcement); `can_purchase` is **redundant** (read at the API layer but the RPC gate blocks staff anyway). Fields kept for now; a future intentional `can_topup` server gate is deferred to a Fix session. | `docs/skills/merchant-staff-billing-reconciliation.md`; `src/app/api/topup/*`, `src/app/api/boosts/*`, `src/lib/merchant.ts` |
| 2026-07-19 | **Top-up is now enforced owner-only *intentionally*** (closes the latent trap from the 2026-07-16 entry). `/api/topup` and `/api/topup/stripe` resolve context via `requireMerchant()` and hard-require `isOwner` (staff → explicit **403**, previously an incidental 404); the `/merchant/topup` page redirects non-owners. `can_topup` remains **deliberately not consulted** — it is owner-settable, and the frozen rule only opens staff billing via a governance (decisions-log) change, never an owner flag. No change to who can top up (still owners only); the block is now by intent, not by the owner-keyed merchant lookup. | `src/app/api/topup/route.ts`, `src/app/api/topup/stripe/route.ts`, `src/app/merchant/(app)/topup/page.tsx`; tests in `src/app/api/topup/**/__tests__/route.test.ts` |
