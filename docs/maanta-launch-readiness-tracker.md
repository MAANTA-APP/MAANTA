...
> below; the register rows are the detail. **All three re-verified against
> production on 2026-08-01 and still failing** — see E17's note and
> `docs/ops/launch-distance-2026-08-01.md`.

> **2026-08-01 — how far from launch?** Excluding mall approval, incorporation
> and the M-Pesa rail, the remaining distance is **one human `supabase db push`
> (E17), one real-device pilot day (E2/E3/E4), and four founder decisions**
> (O2, Stripe sandbox-vs-live, E10 env confirmation, agent/incident roster).
> Full analysis with re-runnable production queries:
> [`docs/ops/launch-distance-2026-08-01.md`](ops/launch-distance-2026-08-01.md).
> M1–M7 and E7/E8 are `GATE (campaign)`, not launch gates — a friends-and-family
> pilot needs none of them.
...
| E11 | Trial-expiry job scheduling confirmed in production Supabase | Engineer | ✅ done | GATE | **2026-07-29 audit found the job was never registered:** pg_cron is installed on prod but `cron.job` had **zero rows** — the old migrations' direct `INSERT INTO cron.job` (with `EXCEPTION … NULL`) silently failed, so `handle_trial_expiry` never ran (101 merchants on Elite trial, none would ever grace/downgrade). Corrective migration `20260729092118_schedule_trial_expiry_cron.sql` uses the supported `cron.schedule()` API. **Applied to prod 2026-07-29** — job `maanta_handle_trial_expiry` is registered and `active` (`0 2 * * *`), function smoke-run clean (0 trials past-due). **Closed 2026-08-01:** the remaining item — the first real nightly run — is confirmed. `cron.job_run_details` shows **three consecutive `succeeded` runs** (2026-07-30, 07-31, 08-01 at 02:00 UTC). Read back from prod, not inferred |
