...
Last updated: 2026-08-02 · Review weekly (Product track, Step 5). Update this
...
>
> **2026-08-02:** a third migration now waits on the same human push —
> `maanta-app/supabase/migrations/20260802120000_nodes_registry.sql` (**D72**),
> which adds the `public.nodes` registry and foreign keys on `deals.node` /
> `merchants.node`. It is numbered above production's ledger max so it cannot be
> silently skipped the way the pause gate was, and it seeds from live data
> before adding the keys so it cannot fail on unexpected production rows.
> Apply steps and verification queries: `docs/ops/supabase-migrations.md`.
...
> (45 files) + 16 SQL suites** *(the count on that date — as of 2026-08-02 it is
> **578 tests across 74 files + 23 SQL suites**; the dated figure is left intact
> rather than overwritten, because this block is a record, not a status line)*,
> lint + typecheck clean. Production is **live and
...
| E6 | M-Pesa STK end-to-end | Engineer | 🔴 blocked | GATE | Blocked on IntaSend API access; code path exists. Escalate credential request weekly. **Updated 2026-08-02 (D70):** the top-up webhook no longer credits a wallet from its request body. IntaSend signs nothing, so a valid challenge only proved the caller knew the shared secret — the body could name any merchant and any amount up to KES 1,000,000. The webhook now supplies only `invoice_id` and the amount, merchant and settled-state are read back from IntaSend's `POST /payment/status/`. **This adds a step to unblocking, not just credentials:** the endpoint contract came from IntaSend's published docs, not a live call, so the first thing to do with a sandbox account is one STK push confirming `/payment/status/` returns an `invoice` carrying `state`, `value`, `currency` and the `api_ref` this app issued. If it differs, the failure is safe — top-ups return 500 and retry rather than over-credit — but M-Pesa will not work. That read-back closes D70 |
