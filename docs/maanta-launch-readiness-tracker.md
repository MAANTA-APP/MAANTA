...
| E12 | Money-path + golden-path automated tests (ENGINEERING_NOTES §8.3/§8.4) | Engineer | ✅ done | — | Merged PR #32 (2026-07-21). `supabase/tests/{golden_path,verify_redemption_money_path,topup_settles_arrears}_test.sql` run in CI `db-tests` against a real Supabase: one-winner double-verify / no double charge, owed@low-balance, unknown → fraud task, settle-first, ledger reconciliation. **Re-verified 2026-07-21** on a Docker-free local Postgres 16 harness (Supabase-compat bootstrap; postgis stubbed to text — the money path never touches it): all 50 migrations apply clean, all **6 SQL suites pass**, alongside 40/40 vitest, clean typecheck, clean `next build` (91 routes). Reproduction steps: `docs/skills/mvp-frozen-spec-verification.md` |
