...
> migrations applied (this session adds a 68th — the trial-expiry cron fix
> `20260729120000`, **pending prod apply**, see E11), Vercel prod deploying from `main` (100%
...
| E11 | Trial-expiry job scheduling confirmed in production Supabase | Engineer | 🟡 in progress | GATE | **2026-07-29 audit found the job was never registered:** pg_cron is installed on prod but `cron.job` had **zero rows** — the old migrations' direct `INSERT INTO cron.job` (with `EXCEPTION … NULL`) silently failed, so `handle_trial_expiry` never ran (101 merchants on Elite trial, none would ever grace/downgrade). Corrective migration `20260729120000_schedule_trial_expiry_cron.sql` uses the supported `cron.schedule()` API. **Apply it to prod, then verify** `SELECT jobname, schedule, active FROM cron.job;` |
