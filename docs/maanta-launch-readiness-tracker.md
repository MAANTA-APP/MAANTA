...
| Shopper redeem at counter (merchant verify) | ✅ | `verify_redemption` RPC: atomic verify + fee debit/arrears. 2026-07-24: success takeover now shows the cashier **"Collect from shopper KES N"** (YOU PAY `amount_kes`), distinct from the KES 30 fee — closes audit drift P12 (`redemption-result.tsx`, verify route; tests in `redemption-result.test.ts`) |
...
| E9 | FX provider replaced with SLA-backed source | Engineer | 🟡 in progress | GATE if non-KES live charges | 2026-07-24: **provider abstraction landed** (`src/lib/fx/` `FxProvider` + open-er-api + static; `currency.ts` consumes it, behaviour unchanged; `fx.test.ts`; `docs/skills/fx-provider.md`). Swapping to a paid/SLA source is now a `configureFxProvider()` call — still needs the real provider + legal FX disclosure. Fine to defer if launch is KES-only |
...
| E14 | Browser golden-path E2E (Playwright: `/demo` → claim → verify → wallet) | Engineer | 🟡 in progress | — | 2026-07-24: **suite authored** (`maanta-app/e2e/golden-path.spec.ts`, `playwright.config.ts`) covering browse→claim→verify (asserts P12 collect line)→ negative code; **self-skips without a live env** so it's not false coverage; opt-in CI job `.github/workflows/e2e.yml` gated on `E2E_BASE_URL` (no lockfile change). Remaining (human): provision live Supabase + Clerk test creds/storage states, then decide CI gating — `docs/ops/e2e-golden-path.md`. Open PR #35 |
