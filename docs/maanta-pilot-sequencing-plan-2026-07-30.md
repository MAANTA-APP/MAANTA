...
| Founder | Config | **Yes** | **`SUPABASE_SERVICE_ROLE_KEY` works** — confirm it is *present*, then prove it functionally. Do **not** reveal, copy, screenshot or paste the value: it bypasses RLS entirely, and a key you never displayed is a key you never leaked | The variable shows as set, and a server-side surface that needs it renders real data — `/admin/billing` showing the Elite cap counter proves the key works without exposing it |
...
| Apply `elite_merchants_100.sql` (PR #112's seed) to production | Inserts 100 BBS Mall merchants with `elite_trial_active = TRUE` and **no `is_demo`**. Each insert trips `trg_enforce_elite_trial_cap`, which stamps **`elite_trial_granted_at`** — and that stamp, on non-demo launch-node rows, is exactly what `elite_trial_cap_status()` counts. All 100 slots gone, and the stamp is never cleared |
