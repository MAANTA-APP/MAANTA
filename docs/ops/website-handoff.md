...
1. ~~**`/api/contact` does not exist.**~~ **RESOLVED — do not re-report.** The audit was right and the enquiries were being discarded: `/contact` called `setSent(true)` with no fetch behind it. Fixed exactly as suggested here — `maanta-app/src/app/api/contact/route.ts` delivers to `admin@maanta.app` via Resend with `reply_to` set to the sender and autoresponds when an email was given, and the page shows its confirmation only after that request succeeds. Drift **D28**, closed. The form then turned out not to render in server HTML at all, which is drift **D41**, also closed. *(Original text: "The production build has `/api/leads`, `/api/waitlist`, `/api/staff`, `/api/support` — but `/contact` renders a form with a Send button. Either it piggybacks another endpoint or enquiries are being discarded. Check first. Resend is already connected to the account and is the obvious fix.")*
