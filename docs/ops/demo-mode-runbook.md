...
>
> **Known gap, not a demo-mode bug:** server-side capture drops events. Four deal
> pages were rendered during this check and two events arrived — `captureServerEvent`
> is called unawaited with no `waitUntil`, so an in-flight ping dies when the Vercel
> instance freezes. Tagging is correct; delivery undercounts. See
> `optruth-demo-release-2026-07-29.md` → "Server-side capture drops events".
