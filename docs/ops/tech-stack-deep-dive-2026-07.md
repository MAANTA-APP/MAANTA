...
Last updated: 2026-07-27 · Status: **repo-grounded assessment** (docs only).  
...
Related skills: [clerk-auth](../skills/clerk-auth.md),
...
| Maps | Leaflet + react-leaflet (shopper browse map) | `package.json` |
| Fonts | DM Sans, Inter, JetBrains Mono via `next/font/google` | `src/app/layout.tsx` |
| PWA manifest | `manifest.webmanifest` — `start_url: /feed`, `display: standalone` | `public/manifest.webmanifest`, linked from root layout |
| Service worker | **Push + notification click only** — no offline asset cache | `public/sw.js`; registered from `InstallPrompt` / feed notification opt-in |
| Install UX | `beforeinstallprompt` bottom sheet | `src/components/install-prompt.tsx` |

**Not in the repo (despite older product assumptions):**

- No `/download` route under `src/app/`.
- No `/app-bootstrap` routing anywhere.
- No `src/lib/roles.ts` — roles and guards live elsewhere (see Auth / roles).

**Role-based routing** (middleware does **not** enforce roles — only Clerk session):
...
Clerk gate: `src/middleware.ts` exports `clerkMiddleware()` on all matched paths.
...
|---|---|
| Provider | **Clerk** (`@clerk/nextjs` ^6.39.6) — sign-in/up UI, session JWT |
| Data authz | **Supabase** third-party JWT verification + RLS / `current_user_id()` helpers — not Supabase Auth for sign-in |
| Strategies | Email + phone OTP via Clerk-hosted UI (`NEXT_PUBLIC_LAUNCH_AUTH_MODE`, default email+phone). Claim path requires a **verified phone** (`currentUserHasVerifiedPhone` in `src/lib/auth.ts`, `/verify-phone`) |
| Social OAuth | **No app wiring** — would be Clerk-dashboard-only if enabled |
| Twilio Verify | Marked legacy/unused in `.env.example` |
| Instance | `cheerful-sailfish-3` (see `supabase/config.toml`, clerk-auth skill) |
...
See `docs/skills/role-permissions.md`.
...
| App API | Next.js route handlers under `src/app/api/*` (~37 routes): claim/verify, deals, top-ups, webhooks (Stripe/IntaSend), admin, push, waitlist, healthz |
...
**Hot-read pattern:** `getLiveDeals(node)` — three capped bucket queries (flash/boosted/standard) + `verified_counts_by_merchant` RPC, wrapped in `unstable_cache` for **30s** per node (`src/lib/data.ts`). Browse/map filters that payload client-side.
...
- Browse/map does **client-side** filtering over the live-deal payload — OK at Node 0 deal counts; painful if live inventory grows large per node.
- Admin/merchant lists use small `.limit(...)` windows, not cursor/keyset pagination.
- No `/download` or bootstrap route — install UX is in-app only.
...
- Production-shaped split: Clerk for identity, Supabase for RLS/data — not a throwaway auth prototype (`docs/skills/clerk-auth.md`).
...
- Placeholder/dev Clerk keys break interactive browser (FAPI “Invalid host”) — already documented in `AGENTS.md`; must stay on a real production project.
...
**Verdict:** **Keep Clerk; harden production plan, SMS strategy, and role separation.** Not “replace auth framework.”
...
- Multi-node reporting and inventory growth not yet first-class.
...
5. Treat full offline SW caching as **optional product work**, not a scale prerequisite — current SW is correctly scoped to push.

### Auth

1. Run a **production Clerk project** with real SMS/email limits and pricing (not placeholder keys).
2. Keep **phone OTP for claim**; prefer email (and later social) for browse/login to control SMS spend.
3. Document an **SMS cost ceiling** and fallback (email-primary login; phone only when claiming) if Clerk SMS becomes prohibitive.
...
2. Sentry **alerts**: spike in 5xx, payment webhook failures (`logWebhookFailure`), claim/verify error rates.
...
- Focus: App Router surfaces, RPC money path (`claim_deal` / `verify_redemption` / ledger), Vitest + `supabase/tests/*.sql`, role guards in `src/lib/{admin,agent,founder,merchant}.ts`.
- Ask them to challenge: pagination, cache invalidation, service-role blast radius, PostgREST cap footguns.

### Infra Advisor (Manchester)

- Focus: Vercel env completeness, Sentry/PostHog alerts, Supabase capacity and backups, scheduling `handle_trial_expiry`, future workers/cron, CI → deploy hygiene (`make db-push` discipline).
...
- Focus: where tech choices hit the till — redeem latency (verify RPC), wallet/top-up reliability (Stripe/IntaSend webhooks), zero-balance deal-create gate, verify-anyway trust UX, push notification reliability (`sw.js` + VAPID).
...
|---|---|
| `/download` + `/app-bootstrap` PWA shell | **Missing** — manifest + push SW + install prompt only |
...
| Auth is still prototype-only | **No** — Clerk + Supabase third-party JWT is production-shaped; SMS **cost** is the scale risk |
