...
### The node registry — `20260802120000_nodes_registry.sql` (D61)
...
- **Verify on prod:**
  ...
  ```
- **Close:** D61 does **not** close on this push alone. It also needs node
  *selection* moved off the compiled array in `src/lib/data.ts`
  (`getSelectedNode`). Record the push against D61 and leave it open.
