...
- [x] All 17 marketing routes go through it — enumerated, not sampled:
      `find src/app/\(marketing\) -name page.tsx` returns 17 and all 17 call it.
      `/pricing` had no metadata and `/merchants/join` could not export any while it
      was a `"use client"` module; both were fixed separately before this step landed
      (**D52**, and the `/pricing` card rebuild), so this pass wraps metadata that
      already existed rather than creating it
